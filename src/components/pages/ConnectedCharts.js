...
const constrainedActivities = ['working', 'commuting']
...
const generateDayActivity = () => {
    ...
    const hasConstrainedActivities = Math.random() < 0.5
    if (hasConstrainedActivities) {
        activities = activities.concat(constrainedActivities)
    }

    activities = activities.concat(
        _.shuffle(niceActivities).slice(
            0,
            1 + Math.round(Math.random() * (niceActivities.length - 1))
        )
    )
...
const rawData = timeDays(from, to).map(dayDate => ({
    date: dayDate,
...
class ConnectedCharts extends Component {
    ...
    handleBarsModeToggle() {
        ...
        this.setState({
            barsMode: barsMode === 'stacked' ? 'grouped' : 'stacked',
        })
    }
    ...
    render() {
        ...
        if (dayFilter === null) {
            ...
            bubbleLegend = (
                <span onClick={this.handleDayFilterClear}>
                    x {dayFilter}
                </span>
            )
        }
                ...
                <div className="page_content">
                    <div className="grid">
                        <div className="grid_item grid_item-2_3">
                            ...
                            <div style={{ height: '200px' }}>
                                <div>
                                    <span onClick={this.handleBarsModeToggle}>
                                        mode: {barsMode}
                                    </span>
                                </div>
                                <ResponsiveBarsD3
